...
export const terminalCommands = {
    ...
    help: {
        ...
        execute: (args, context) => {
            ...
            const basicCommands = ['help', 'clear', 'whoami', 'pwd', 'ls', 'cd'];
            ...
            output += "\n=== Data Commands ===\n";
            ['work', 'about', 'skills', 'smallprojects', 'pcspecs'].forEach(cmd => {
                if (terminalCommands[cmd]) {
                    output += `  ${cmd.padEnd(15)} - ${terminalCommands[cmd].description}\n`;
                }
            });
            ...
            output += "\n=== Utility Commands ===\n";
            ['contact', 'github', 'resume', 'social'].forEach(cmd => {
                if (terminalCommands[cmd]) {
                    output += `  ${cmd.padEnd(15)} - ${terminalCommands[cmd].description}\n`;
                }
            });
    ...
    resume: {
        ...
        execute: (args) => {
            if (args.includes('open')) {
                ...
                return "OPEN_RESUME:https://drive.google.com/file/d/1KgRhoBu6uCN5dYcK4SXuFvgp6V8kZpc7/view";
    ...
    github: {
        ...
        execute: (args) => {
            if (args.includes('profile')) {
                return "GitHub Profile: https://github.com/ManishGupta-x\nPublic Repositories: 25+\nLanguages: JavaScript, Python, C++\nActive Contributor";
            } else if (args.includes('repos')) {
                return "Notable Repositories:\n• Jarvis - Discord Music Bot\n• Image Steganography Tool\n• Smart Traffic Management\n• Telephone Diary (C++)\n\nView all: https://github.com/ManishGupta-x?tab=repositories";
    ...
    // Achievement Command  
    achievements: {
        description: "Display achievements and milestones",
        usage: "achievements",
        category: "personal",
        execute: () => {
            let output = "=== Achievements & Milestones ===\n\n";
            achievements.forEach(achievement => {
                output += `${achievement.number} ${achievement.label}\n`;
                output += `Platform: ${achievement.platform}\n\n`;
            });
            return output.trim();
        }
    },
