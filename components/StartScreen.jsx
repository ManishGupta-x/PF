    ...
    <div className={`fixed inset-0 flex items-center justify-center bg-cream z-50 overflow-hidden transition-opacity duration-1000 ${
      ...
      <div className='relative z-10 flex'>
        {letters.map((letter, index) => (
          <span
            key={index}
            className={`text-foreground text-[13rem] font-bold font-geo transition-all duration-500 ${
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 pointer-events-none">
        ...
        <div className={`absolute -top-20 -left-32 w-2 h-2 rounded-full bg-brown transition-all duration-1000 delay-1000 ${
          animationPhase === 'entering' ? 'opacity-0 scale-0' :
          animationPhase === 'displaying' ? 'opacity-40 scale-100' :
          'opacity-0 scale-0'
        }`}></div>
        <div className={`absolute -bottom-16 right-24 w-3 h-3 rounded-full bg-medium-tan transition-all duration-1000 delay-1200 ${
          animationPhase === 'entering' ? 'opacity-0 scale-0' :
          animationPhase === 'displaying' ? 'opacity-30 scale-100' :
          'opacity-0 scale-0'
        }`}></div>
        <div className={`absolute top-8 -right-40 w-1.5 h-1.5 rounded-full bg-accent-brown transition-all duration-1000 delay-1400 ${
          animationPhase === 'entering' ? 'opacity-0 scale-0' :
          animationPhase === 'displaying' ? 'opacity-50 scale-100' :
          'opacity-0 scale-0'
        }`}></div>
        <div className={`absolute -top-12 right-20 w-2.5 h-2.5 rounded-full bg-light-tan transition-all duration-1000 delay-1600 ${
          animationPhase === 'entering' ? 'opacity-0 scale-0' :
